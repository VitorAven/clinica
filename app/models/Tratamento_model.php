...
class Tratamento_model extends CI_Model {
    ...
    public function salvar($param) {
        ...
        try {
            ...
            $this->adicionarTratamento($param);
            return $id_pessoa;
        } catch (Exception $exc) {
            return null;
        }
    ...
    public function listarTodosTratamento() {
        unset($query);
    ...
    public function adicionarTratamento($param) {
        $this->db->start_cache();

        $param['nr_salario'] = str_replace('.', '', $param['nr_salario']);
        $param['nr_salario'] = str_replace(',', '.', $param['nr_salario']);
        //Adiciona o medico na tabela de Tratamento 
        ...
        if (!empty($param['id_pessoa'])) {
            $funPessoa = $this->getTratamentoByIdPessoa($param['id_pessoa']);
    ...
    public function getTratamentoByIdPessoa($id_pessoa) {
        if (empty($id_pessoa)) {
            return null;
        }
        unset($query);
